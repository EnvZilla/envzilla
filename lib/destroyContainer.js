 ...
 */
export async function destroyContainer(containerId, prNumber, options = {}) {
    const result = {
        success: false,
        containerId,
    ...
    const isValidId = typeof containerId === 'string' && (/^[a-f0-9]{64}$/.test(containerId) || // Full 64-char hex
 ...
 */
async function destroyAssociatedImages(containerId, prNumber, result) {
    try {
 ...
 */
async function cleanupByContainerName(containerName, result) {
    try {
 ...
 */
export async function destroyByPRNumber(prNumber) {
    const results = [];
    try {
...
//# sourceMappingURL=destroyContainer.js.map